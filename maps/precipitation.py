...
def precipitation(startYear,endYear,startMonth,endMonth):
    dataset = ee.ImageCollection('UCSB-CHG/CHIRPS/DAILY').filter(ee.Filter.date(str(startYear)+'-'+str(startMonth)+'-'+'28', str(endYear)+'-'+ str(endMonth)+'-'+'28'))
